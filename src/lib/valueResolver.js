...
const processNames = require('./namesProcessor');
    ...
    switch (left.type) {
        case 'Identifier': {
            ...
            if (!leftReference) {
                ...
                let targetScope = utils.ast.bubble(left.scope);
        ...
        case 'MemberExpression':
            // a.b = ...
            let keyObject = prepareMemberExpression(left, walker);
...
function prepareMemberExpression(token, walker) {
    let objectToken = token.object;
    let keyToken = token.property;
    let computed = token.computed;
    let obj = resolveToken(objectToken, walker);
    let key;
...
function handleObjectExpression(token) {
    ...
    for (let i = 0; i < token.properties.length; i++) {
        let computed = token.properties[i].computed;
        let keyToken = token.properties[i].key;
        let valueToken = token.properties[i].value;
        let key;
...
function resolveBinaryExpression(token, walker) {
    ...
    let leftValue = handleSide(token.left, walker);
    let rightValue = handleSide(token.right, walker);
    let result;
    ...
    switch (token.operator) {
        ...
        case '>=':
            ...
            break;
        case 'in':
            let rightType = typeof rightValue.value;
...
function resolveUnaryExpression(token, walker) {
    ...
    let argumentValue = handleArgument(token.argument, walker);
    let result;
    ...
    switch (token.operator) {
        ...
        case 'delete':
            if (token.argument.type === 'MemberExpression') {
                let keyObject = prepareMemberExpression(token.argument, walker);
...
function resolveUpdateExpression(token, walker) {
    ...
    if (token.argument.type === 'MemberExpression') {
        ...
        let reference = token.argument.scope.getReference(token.argument.name);
...
function handleClassToken(token) {
    ...
    function resolveMethod(token, fn) {
        let method = resolveClassMethod(token);
    ...
    let fn = handleFunctionToken(token);
...
function resolveCallExpression(token) {
    let callee = resolveToken(token.callee).value;
    let result;
    let i;
    ...
    if (typeof callee === 'function' && callee.token) {
        ...
        let args = { length: token.arguments.length };

        for (i = 0; i < token.arguments.length; i++) {
            // todo support SpreadElement
            let arg = token.arguments[i];
        ...
        for (i = 0; i < callee.token.params.length; i++) {
            ...
            let param = callee.token.params[i];
...
function resolveToken(token, walker) {
    ...
    switch (token.type) {
        ...
        case 'ThisExpression':
            let current = token.scope;

            while (current && current.thisRef === null) {
                current = current.parent;
            }

            if (current) {
                return { resolved: true, value: current.thisRef };
            }
